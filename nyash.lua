...
local window = lib:MakeWindow({Name = "menu cus u shit"})
local main = window:MakeTab({Name = "get good bozo"})

main:AddToggle({Name = "make you hit shots even if your shit", Default = false, Callback = function(v)
    if v then
        game.StarterGui:SetCore("SendNotification", {Title = "aimbot enabled", Text = "just jump when your character goes green", Duration = 5, Button1 = "dick"})
    end
    _G.Aimbot = v
end})
main:AddToggle({Name = "gimme that ball bozo", Default = false, Callback = function(v)
    if v then
        game.StarterGui:SetCore("SendNotification", {Title = "reach enabled", Text = "this will increase block range and grab range", Duration = 5, Button1 = "dick"})
    end
    _G.Reach = v
end})
main:AddToggle({Name = "smh ill guard for you", default = false, Callback = function(v)
    if v then
        game.StarterGui:SetCore("SendNotification", {Title = "smh ill guard for you", Text = "Press U to go to the nearest balls", Duration = 5, Button1 = "dick"})
    end
    _G.Autogaurd = v
end})
main:AddSlider({Name = "ill make you walk faster too", Min = 16, Max = 19, Default = 16, Color = Color3.fromRGB(80, 80, 255), Increment = 0.1, Callback = function(v)
    _G.WS = v
end})
